...
//     https
//       .get(`https://wooordhunt.ru/get_tips.php?abc=${word}`, resp => {
//         let data = "";
...
//         resp.on("end", () => {
//           try {
//             resolve(JSON.parse(data).tips[0]);
//           } catch (error) {
//             resolve({ w: word, t: null });
//           }
//         });
//       })
//       .on("error", err => {
...
const createPromise2 = word => {
  return new Promise((resolve, reject) => {
    https
      .get(`https://www.babla.ru/английский-русский/${word}`, resp => {
        let data = "";
        ...
        resp.on("end", () => {
          let regres,
            t = [],
            regexp = /перевод на английский">([а-яё]+)/g;
          while ((regres = regexp.exec(data))) {
            t.push(regres[1]);
          }
          if (t.length > 0) {
            resolve({ w: word, t: t.toString() });
          } else {
            resolve({ w: word, t: null });
          }
        });
